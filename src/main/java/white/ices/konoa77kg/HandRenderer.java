...
public class HandRenderer {
    ...
    public HandRenderer(Minecraft mcIn) {
        this.mc = mcIn;
    ...
    private void renderArm(EnumHandSide p_187455_1_) {
        ...
        RenderFatPlayer3 renderplayer = Konoa77kg.render;
        GlStateManager.pushMatrix();
        ...
        if (p_187455_1_ == EnumHandSide.RIGHT) {
            renderplayer.renderRightArm((EntityPlayer)this.mc.player);
        } else {
            renderplayer.renderLeftArm((EntityPlayer)this.mc.player);
        }
        GlStateManager.popMatrix();
    }
    ...
    private void renderArmFirstPerson(float p_187456_1_, float p_187456_2_, EnumHandSide p_187456_3_) {
        ...
        GlStateManager.disableCull();
        if (flag) {
            renderplayer.renderRightArm((EntityPlayer)entityPlayerSP);
        } else {
            renderplayer.renderLeftArm((EntityPlayer)entityPlayerSP);
        }
        GlStateManager.enableCull();
    }
    ...
    public void renderItemInFirstPerson(AbstractClientPlayer player, float p_187457_2_, float p_187457_3_, EnumHand hand, float p_187457_5_, ItemStack stack, float p_187457_7_) {
        ...
        if (stack.isEmpty()) {
            ...
            if (player.isHandActive() && player.getItemInUseCount() > 0 && player.getActiveHand() == hand) {
                ...
                switch (stack.getItemUseAction()) {
                    ...
                    case BOW:
                        ...
                        f5 = stack.getMaxItemUseDuration() - this.mc.player.getItemInUseCount() - p_187457_2_ + 1.0F;
                        f6 = f5 / 20.0F;
            ...
            renderItemSide((EntityLivingBase)player, stack, flag1 ? ItemCameraTransforms.TransformType.FIRST_PERSON_RIGHT_HAND : ItemCameraTransforms.TransformType.FIRST_PERSON_LEFT_HAND, !flag1);
        }
        GlStateManager.popMatrix();
    }
    ...
    public void renderOverlays(float partialTicks) {
        ...
        if (this.mc.player.isEntityInsideOpaqueBlock()) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                BlockPos blockpos = new BlockPos(d0, d1 + entityPlayerSP.getEyeHeight(), d2);
                IBlockState iblockstate1 = this.mc.world.getBlockState(blockpos);
                if (iblockstate1.isBlockNormalCube()) {
                    iblockstate = iblockstate1;
                    overlayPos = blockpos;
                }
            }
            if (iblockstate.getRenderType() != EnumBlockRenderType.INVISIBLE)
                if (!ForgeEventFactory.renderBlockOverlay((EntityPlayer)this.mc.player, partialTicks, RenderBlockOverlayEvent.OverlayType.BLOCK, iblockstate, overlayPos))
        ...
        if (!this.mc.player.isSpectator()) {
            if (this.mc.player.isInsideOfMaterial(Material.WATER))
                if (!ForgeEventFactory.renderWaterOverlay((EntityPlayer)this.mc.player, partialTicks))
                    renderWaterOverlayTexture(partialTicks);
            if (this.mc.player.isBurning())
                if (!ForgeEventFactory.renderFireOverlay((EntityPlayer)this.mc.player, partialTicks))
    ...
    public void updateEquippedItem() {
        ...
        if (entityplayersp.isRowingBoat()) {
            ...
            this.equippedProgressOffHand += MathHelper.clamp((!requipO ? 1 : 0) - this.equippedProgressOffHand, -0.4F, 0.4F);
        }
